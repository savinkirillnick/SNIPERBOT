...
if ($method == 'getRules') {
	...
	for ($i = 0; $i < $count; $i++) {
		if ($fcontents['symbols'][$i]['baseAsset'] != 123) {
			...
			$rules .= "\"minQty\":".$fcontents['symbols'][$i]['filters'][1]['minQty'].",";
			$rules .= "\"maxQty\":".$fcontents['symbols'][$i]['filters'][1]['maxQty'].",";
			$rules .= "\"aroundPrice\":".abs(log10($fcontents['symbols'][$i]['filters'][0]['tickSize'])).",";
			$rules .= "\"aroundQty\":".abs(log10($fcontents['symbols'][$i]['filters'][1]['stepSize'])).",";
			$rules .= "\"minSum\":0,";
			$rules .= "\"maxSum\":0";
			$rules .= "},";
		}
